...
export function SectionDesignRoom({
  ...
  const activeColour = colourControls.find(control => control.id === activeColourId) || null;
  const activeColourValue = activeColour
    ? getColorInputValue(activeColour.value || '', activeColour.fallback || '#050505')
    : '#050505';
  const documentColours = Array.from(new Set(colourControls.map(control => (
  ...
  return (
    <div className="editor-section-design-room">
      ...
      {fields.length > 0 ? (
        <fieldset className="editor-section-setting-group">
          <legend>
