      ...
      if (emailKey) {
        downstreamWrites.push(FirebaseSDK.setDoc(
          FirebaseSDK.doc(db, 'artifacts', appId, 'clientAccess', emailKey, 'bookings', bookingId),
          {
            ...
            scheduleType: updates.scheduleType ?? existingBooking?.scheduleType ?? existingBooking?.serviceScheduleType ?? 'appointment',
            serviceScheduleType: updates.serviceScheduleType ?? existingBooking?.serviceScheduleType ?? existingBooking?.scheduleType ?? 'appointment',
            scheduleResourceId: updates.scheduleResourceId ?? existingBooking?.scheduleResourceId ?? '',
            scheduleResourceName: updates.scheduleResourceName ?? existingBooking?.scheduleResourceName ?? '',
            scheduleSessionId: updates.scheduleSessionId ?? existingBooking?.scheduleSessionId ?? '',
            scheduleSessionName: updates.scheduleSessionName ?? existingBooking?.scheduleSessionName ?? '',
            partySize: updates.partySize ?? existingBooking?.partySize ?? '',
