...
const bookingFilterSpecs = [
  { id: 'upcoming', label: 'Upcoming', countKey: 'upcoming', iconKey: 'calendarCheck' },
  { id: 'review', label: 'Review', countKey: 'review', iconKey: 'bell' },
  { id: 'confirmed', label: 'Confirmed', countKey: 'confirmed', iconKey: 'check' },
  { id: 'waitlist', label: 'Waitlist', countKey: 'waitlist', iconKey: 'clock' },
  { id: 'history', label: 'History', countKey: 'history', iconKey: 'history' },
  { id: 'all', label: 'All', countKey: 'total', iconKey: 'layers' }
];
...
const safeDateFromKey = (dateKey, fallback) => {
...
export const sortBookingRows = (rows = [], bookingSort = 'newest') => {
  const nextRows = [...rows];
...
const createPeriodConfig = ({ bookingCustomRange = {}, monthEnd, monthStart, todayStart, weekEnd }) => {
  const customFrom = bookingCustomRange.from || getLocalDateStr(todayStart);
  const customTo = bookingCustomRange.to || customFrom;
  const customStart = safeDateFromKey(customFrom, todayStart);
  const customEnd = safeDateFromKey(customTo, customStart);
  ...
  return {
    ...
    custom: {
      ...
      label: 'Custom',
      periodName: 'Custom',
      rangeLabel: `${formatRangeDate(customStart)} - ${formatRangeDate(customEnd)}`,
      start: customStart,
      end: customEnd
    }
  };
...
export const createBookingDeskModel = ({
  bookingCustomRange,
  bookingDeskPeriod = 'all',
  ...
  const filters = bookingFilterSpecs.map(filter => ({
    id: filter.id,
    label: filter.label,
    ...
    metrics: [
      {
        ...
        hint: activePeriod.id === 'all' ? 'All time' : activePeriod.id === 'day' ? 'Today' : activePeriod.id === 'week' ? 'This week' : 'This month',
        iconKey: 'calendarCheck'
      },
      { label: 'Needs Review', value: reviewRecords.length, hint: `${pending} pending / ${waitlist} waitlist`, iconKey: 'bell' },
      { label: 'Confirmed', value: confirmedRecords.length, hint: `${bookingRate}% booking rate`, iconKey: 'checkCircle' },
      { label: 'History', value: historyRecords.length, hint: `${declinedRecords.length} declined`, iconKey: 'history' }
    ]
  };
};
