import { CalendarDays, Clock3, CreditCard, Sparkles, UserRound } from 'lucide-react';
...
const SummaryRow = ({ compact = false, label, meta, value, settings }) => {
  ...
  const Icon = summaryIcons[labelKey] || Sparkles;
  ...
  return (
    <div className={`booking-summary-row ${compact ? 'booking-cart-summary-row' : ''} flex items-start gap-3 rounded-xl border px-3 py-3`} style={{ backgroundColor: compact ? '#ffffff' : withColorAlpha(settings.headingColor || '#000000', 2, '#000000'), borderColor: settings.pageBorderColor || withColorAlpha(settings.headingColor || '#000000', 5, '#000000') }}>
      ...
      <span className={`booking-summary-copy ${compact ? 'booking-cart-summary-copy' : ''} min-w-0`}>
        <span className="booking-summary-label block text-[9px] font-black uppercase tracking-[0.24em]" style={{ color: withColorAlpha(settings.bodyColor || '#666666', 50, '#666666'), fontFamily: getFontFamily(settings.bodyFontFamily || settings.fontFamily) }}>{label}</span>
        <span className="booking-summary-value mt-0.5 block text-sm font-black leading-tight" style={{ color: settings.headingColor, fontFamily: getFontFamily(settings.headingFontFamily || settings.fontFamily) }}>{value || 'Not selected'}</span>
        {meta && <span className="booking-summary-meta mt-1 block text-[11px] font-bold leading-tight" style={{ color: settings.bodyColor, fontFamily: getFontFamily(settings.bodyFontFamily || settings.fontFamily) }}>{meta}</span>}
      </span>
    </div>
  );
};
  ...
  return (
    <aside data-summary-layout={isCartSummary ? 'table' : undefined} className={`booking-checkout-summary ${isCartSummary ? 'booking-cart-summary' : ''} rounded-2xl border p-4 md:p-5`} style={{ backgroundColor: settings.pageSurfaceColor || '#ffffff', borderColor: settings.pageBorderColor || '#0000001A', fontFamily: getFontFamily(settings.bodyFontFamily || settings.fontFamily) }}>
      ...
      <div className={`booking-summary-total ${isCartSummary ? 'booking-cart-summary-total' : ''} mt-4 flex items-center justify-between border-t pt-4`} style={{ borderColor: settings.pageBorderColor || '#0000001A', ...(isCartSummary ? { backgroundColor: '#ffffff', display: 'inline-flex', justifyContent: 'flex-start' } : {}) }}>
        <span className="text-[10px] font-black uppercase tracking-[0.28em]" style={{ color: withColorAlpha(settings.bodyColor || '#666666', 50, '#666666'), fontFamily: getFontFamily(settings.bodyFontFamily || settings.fontFamily) }}>Total:</span>
        <span className="text-xl font-black tracking-tight" style={{ color: settings.headingColor, fontFamily: getFontFamily(settings.headingFontFamily || settings.fontFamily) }}>{price || 'Confirmed after review'}</span>
      </div>
    </aside>
  );
};
